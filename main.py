...
from Modules.ErrorHandling import set_error, ERROR_LIST, ASErr
import logging
    ...
    while not type(res) is new_type and not res == '':
        try:
            ...
            if mes.strip().lower() == 'end':
                set_error(code=1001, message='End was requested')
                sys.exit(0)
            res = new_type(mes)
        except ValueError:
...
def tst_ip(with_subnet=True) -> [bool, str or None]:
    """
...
def request_ip(message, with_subnet=True):
    ...
    if is_ip:
        ...
        set_error(code=-500, message='Ip went wrong')
        raise Exception


def dhcp() -> str or None:
    """
...
def nat_overload() -> str or None:
    ...
    for i in range(0, __MAXIMUM_ATTEMPT):
        ...
        if acl_number < 1 or (99 < acl_number < 1000) or acl_number > 2699:
            set_error(code=-501, description='Invalid access list number. Please enter a number between 1 and'
...
def main():
    ...
    try:
        parser = argparse.ArgumentParser(description='Automate configuration of Cisco switches.')
        parser.add_argument('-k', action='store_true', help='Perform housekeeping tasks on the machine')
        parser.add_argument('-st', action='store_true', help='Create a static trunk port')
        parser.add_argument('-dtd', action='store_true', help='Create a dynamic desirable trunk')
        parser.add_argument('-dta', action='store_true', help='Create a dynamic auto trunk')
        parser.add_argument('-dhcp', action='store_true', help='Create a DHCP in the router')
        parser.add_argument('-vdhcp', action='store_true', help='Create a DHCP for the VLAN')
        parser.add_argument('-mvdhcp', type=int, metavar='N', help='Create multiple DHCP for VLANs')
        parser.add_argument('-dpdhcp', action='store_true', help='Create a dot1q port and a DHCP with it')
        parser.add_argument('-mdphcp', type=int, metavar='N',
                            help='Create multiple dot1q ports and a DHCP with each one')
        parser.add_argument('-dot', action='store_true', help='Create a dotq on an interface')
        parser.add_argument('-str', action='store_true', help='Create static routes')
        parser.add_argument('-ospf', action='store_true', help='Create OSPF routes')
        parser.add_argument('-acl', action='store_true', help='Activate ACL')
        parser.add_argument('-nato', action='store_true', help='Activate NAT Overload')
        parser.add_argument('-v', action='store_true', help='Set VLAN')

        args = parser.parse_args()

        # Perform the corresponding action based on the provided flags
        all_cmd = []
        ...
        if args.dtd:
            all_cmd.extend(
                Switch.set_dynamic_trunking_desirable(typed_input('Enter the port to dynamically desirable trunk: ')))

        if args.dta:
            all_cmd.extend(Switch.set_dynamic_trunking_auto(typed_input('Enter the port to dynamically auto trunk: ')))

        if args.dhcp:
            all_cmd.extend(dhcp())

        if args.mvdhcp:
            number_of_pool = args.mvdhcp
            for e in range(number_of_pool):
                all_cmd.extend(dhcp())

        if args.vdhcp:
            all_cmd.extend(vlan_dhcp())

        if args.mdphcp:
            pools_amount = args.mdphcp
